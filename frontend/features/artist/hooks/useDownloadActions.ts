  ...
  const downloadArtist = useCallback(
    async (artist: Artist | null) => {
      ...
      try {
        ...
        const message = error instanceof Error ? error.message : 'Failed to download artist';
        toast.error(message, {
          id: `download-${artist.mbid}`,
        });
      }
  ...
  const downloadAlbum = useCallback(
    async (album: Album, artistName: string, e: React.MouseEvent) => {
      ...
      try {
        ...
        const message = error instanceof Error ? error.message : 'Failed to download album';
        toast.error(message, {
          id: `download-${mbid}`,
        });
      }
